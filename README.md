...
Student lab to develop a simple but complete video calling application with WebRTC. The frontend is implemented in Javascript with a simple HTML page. The signaling server is implemented in Python with Flask and Flask-Socket.io. 

This repository provides a skeleton of the final application without the actual behavior. The sections below provide the instructions for the student to guide them step by step towards the final and complete implementation.
...
<img src="images/webpage.png" height="150px">
...
1. Both peers request the web site from the signaling server where the solution is deployed. The web site consists of the page `static/index.html` and the Javascript implementation `static/webrtcclient.js`.
1. Both peers establish a Socket.io connection with the server. 
1. The first peer (the *Caller*) sends a Socket.io message `join(room_name)` to the server. The servers adds it the the Socket.io room and responds with a `create` message, letting the peer know that it's the Caller.
1. The second peer (the *Callee*) sends a `join(room_name)` message to the server. The server adds it to the room and responds with a `joined` message. The server also sends a `new_peer` message to the Caller (other peer in the room). 
1. Upon reception of the `new_peer` message, the Caller initiates the RTCPeerConnection establishment using the WebRTC API. This is where the WebRTC call establishment starts.
1. The Caller and the Callee exchange WebRTC-generated SDP offers and answers as well as ICE candidates through the server using Socket.io messages of type `invite(SDP offer)`, `ok(SDP answer)`and `ice_candidate(candidat)`. 
...
1. To terminate the call, a peer locally tears down the RTCPeerConnecton and the DataConnection using the WebRTC API. It sends a `bye(room_name)` message to the server, the server removes it from the Socket.io room and forwards the `bye` message to the other peer.
1. Upon reception of the `bye(room_name)` message, the other peer tears down its WebRTC connections. It responds with a `bye(room_name)` message and gets removed from the Socket.io room, too. The server does not forward this message to the first peer, since it has already been removed from the room.
...
Read through the `static/index.html`, `static/webrtcclient.js` and the `app.py` files. The Javascript file implements the WebRTC client in the browser. The Python file implements the Web server and Socket.io signaling server.
...
Once the user clicks the call button, the `call` function of the WebRTC client is called. This function is already implemented. Read this function carefully to understand what is does. 
...
* Try using `getUserMedia` to obtain the media flow from the local camera. If your are using two browsers on the same computer, this may raise an exception since only one browser can use the camera. If there is an exception, use [getDisplayMedia](https://developer.mozilla.org/en-US/docs/Web/API/MediaDevices/getDisplayMedia) to create a screen-sharing stream instead.
...
This implementation uses [Flask-socketio](https://flask-socketio.readthedocs.io/en/latest/) on the server side and [Socket.IO](https://socket.io/docs/v2/) on the client side to establish a bidirectional connection for signaling between the peers and the server. SocketIO is based on Websockets. The HTML file includes the SocketIO library as script. We can therefore use this API in the `static/webrtcclient.js` file. 

### Function `create_signaling_connection`

This function uses the SocketIO library to establishes a SocketIO connection with the server. This is extremly easy ([documentation](https://socket.io/docs/v2/client-initialization/)): simply call the `io()` function provided by the SocketIO library. 
...
* `created(room)`: Server --> Peer: sent by the server after a `join` message to indicate that the peer is the first member of the room. This peer this becomes the Caller and will be responsible for initiating the negotiation.
...
This function is called by the `call` function when the call button is clicked. It asks the user for a room name (conference name). It the has to send a `join` message to the server:

* See the [Socket](https://socket.io/docs/v2/client-api/#Socket) documentation of the SocketIO client API for an example how to use `Socket.emit` to send a message to the server.
...
The video call is identified by a room name. This uses the concept of SocketIO rooms. Read the [Flask-socketio documentation](https://flask-socketio.readthedocs.io/en/latest/), section "Rooms", to understand this concept. 

Separate rooms allow the server to manage several different calls without mixing up the participants. E.g., it can easily forward a message received from one peer to the other peer by sending it to all other peers in the room.
...
```
creates as Socket IO server. The following two functions `handle_connect` and `handle_disconnect` simple serve for debugging and display a message when a client connects or disconnects.
...
* If there are more than one members, emit à `full` message with the room_name as argument to inform the client that it has been refused.

### Function `p2p_message`

All other message handlers such as `invite` or `ok` are much simpler. They mostly have to be forwarded to the peer without any processing by the server.
...
To test it, start the application, use two browsers and click the call button in each of them. The `join` messages should be correctly handled by the clients and the server: the first client (Caller) receives a `created` and `new_peer` message, the second client (Callee) receives a `joined` message. A third client would receive a `full` message.

Implement signaling handlers for peer connection negotiation
------------------------------------------------------------

Once both peers are connected to the same 'room', they use the signaling server to exchange the parameters to set up the direct peer-to-peer peerConnection. This is similar to SIP allowing the peers to negotiate the peer-to-peer RTP flow parameters.

### Function `handle_new_peer`

The Caller receives the `new_peer` message from the server when a second peer joins the room. The first thing to do is to create the dataChannel, since this has to be done before creating a peerConnection offer.

Complete the rest of the function:

* Use the [createOffer](https://developer.mozilla.org/en-US/docs/Web/API/RTCPeerConnection/createOffer) method without any options to create a local SDP offer. Since this method returns a Javascript promise, use the `await` syntax to wait for completion.
...
The event and message handlers have already been connected in the function `add_peerconnection_handlers`. Now we have to implement the handlers.

### Function `handle_local_icecandidate`

Handle the `onicecandidate` event on the peerConnection: if there is a new ICE candidate, send an 'ice_candidate' message with the candidate to the signaling server (see [documentation](https://developer.mozilla.org/en-US/docs/Web/API/RTCPeerConnection/onicecandidate)).
...
In `static/webrtcclient.js`, section **5. Functions to establish and use the DataChannel**.
...
In `static/webrtcclient.js`, section **6. Functions to end call**.

### Signaling message handler

Configure a Websocket handler for a `bye` message to call the `hangUp` function.
